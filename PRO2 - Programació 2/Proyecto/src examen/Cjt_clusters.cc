...
void Cjt_clusters::nuevo_cluster(Cluster& minimo)
{
    ...
    // Hojas:
    clusters[cluster_actual].hojas = clusters[x].hojas + clusters[y].hojas;
...
{
    double d_AB = distancias[x][y];
    int h_A = clusters[x].hojas;
    int h_B = clusters[y].hojas;
    
    double divisor = (double) (h_A + h_B);
    
    double sustraendo = h_A * h_B * d_AB / divisor / divisor;
    
    for (int i = 0; i < cluster_actual; ++i)
    {
        double d_AD = distancias[x][i];
        double d_BD = distancias[y][i];
        
        double numerador = h_A * d_AD + h_B * d_BD;

        double d = numerador / divisor - sustraendo;
